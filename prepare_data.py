...
sys.path.insert(0, abspath("."))
sys.path.insert(0, abspath("jittor_code"))
...
parser.add_argument('--fixsizeW', type=int, default=None, help='fix resize of images and annotations')
parser.add_argument('--fixsizeH', type=int, default=None, help='fix resize of images and annotations')
...
for idx, label_file in enumerate(labels_files):
    ...
    if isfile(join(image_dir, filename+".png")):
        im = cv2.imread(join(image_dir, filename+".png"))
        im = cv2.resize(im, (args.fixsizeW, args.fixsizeH))
    else:
        print("Warning: image %s doesnt exist!" % join(image_dir, filename+".png"))
        continue
...
#            annotation = LineAnnotation(size=[H, W], divisions=args.num_directions, lines=lines)
        ...
        if args.fixsizeW is not None:
            newH = nearest8(args.fixsizeH)
            newW = nearest8(args.fixsizeW)
        else:
            newH = nearest8(H)
            newW = nearest8(W)
        ...
        cv2.imwrite(save_name + '.png', im)
        cv2.imwrite(save_name + '_p_label.png', hough_space_label*255)
        cv2.imwrite(save_name + '_vis.png', vis)
        cv2.imwrite(save_name + '_mask.png', mask*255)
filelist.close()
# print(stastic)
